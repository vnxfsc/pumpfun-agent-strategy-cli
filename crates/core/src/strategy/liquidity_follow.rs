...
#[derive(Debug, Clone)]
pub struct LiquidityFollowStrategyConfig {
...
impl Default for LiquidityFollowStrategyConfig {
    fn default() -> Self {
        Self {
            max_age_secs: 55,
            min_buy_count: 4,
            min_unique_buyers: 4,
            min_total_buy_lamports: 1_500_000_000,
...
#[derive(Debug, Clone)]
pub struct LiquidityFollowStrategy {
    config: LiquidityFollowStrategyConfig,
    pending_entries: HashSet<String>,
    pending_exits: HashSet<String>,
    entry_sell_counts: HashMap<String, u64>,
}

impl LiquidityFollowStrategy {
    pub fn new(config: LiquidityFollowStrategyConfig) -> Self {
        Self {
            config,
    ...
    fn maybe_entry(
        ...
        if (mint_state.real_sol_reserves as u128) < self.config.min_total_buy_lamports {
            return None;
        }
        ...
        Some(OrderRequest::BuyForLamports {
            ...
            reason: format!(
                "liquidity_follow age={}s buys={} unique_buyers={} real_sol_reserves={} net_flow={} ratio={:.2}",
                age_secs,
                mint_state.buy_count,
                mint_state.unique_buyer_count(),
                mint_state.real_sol_reserves,
    ...
    fn maybe_exit(
        ...
        if reason.is_none() && post_entry_sells >= self.config.exit_on_sell_count {
            reason = Some(format!(
                "liquidity_drain sells_since_entry={}",
                post_entry_sells
            ));
        }
...
impl Strategy for LiquidityFollowStrategy {
    fn metadata(&self) -> StrategyMetadata {
        StrategyMetadata {
            name: "liquidity_follow_strategy",
        }
    }
